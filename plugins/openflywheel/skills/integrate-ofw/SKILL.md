...
1. **Instrument the real path.** Find the command, service, or worker that
   actually starts Hermes. Do not create a parallel demonstration agent.
...
3. **Do not change behavior.** This workflow may add an OFW declaration and
   revision attribution. It must not rewrite prompts, tools, skills, or Hermes
   control flow.
...
6. **Verify with a real run.** Integration is complete only when one real Hermes
...
2. Identify the git root and the active Hermes assets:
...
7. Add revision attribution around the real Hermes run using
   `propagate_attributes` and metadata key `ofw.harness.revision`. Do not add a
   second root trace.
8. Run one real Hermes request, flush the existing Langfuse client as required
...
- Stop before editing when the active Hermes entry point or prompt cannot be
...
- the real Hermes command/request used for verification;
