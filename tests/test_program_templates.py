...
def test_packaged_program_template_matches_plugin_asset(name: str) -> None:
    plugin_path = (
        Path(__file__).parents[1] / "plugins/openflywheel/program_templates" / name
    )
...
def test_itsm_program_routes_failure_mining_to_local_workspace_artifacts(
    ...
    content = (
        files("ofw.preparation.templates")
        .joinpath("itsm.md")
...
def test_failure_pattern_miner_skill_is_packaged() -> None:
    skill = (
        ...
        / "plugins/openflywheel/skills/failure-pattern-miner/SKILL.md"
    )
